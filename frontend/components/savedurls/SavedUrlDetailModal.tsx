...
const SavedUrlDetailModal: React.FC<SavedUrlDetailModalProps> = ({
  ...
  const evidenceExtraction =
    (url as any)?.tagsMetaRaw?.tagger?.extraction ?? null;
  ...
  return createPortal(
    <div className="saved-url-detail-modal fixed inset-0 z-50 overflow-y-auto bg-slate-950/28 backdrop-blur-[3px]">
      <div className="saved-url-detail-modal__frame flex min-h-full items-start justify-center p-4 md:p-6">
        <div
          ref={dialogRef}
          role="dialog"
          aria-modal="true"
          aria-labelledby={dialogTitleId}
          className="saved-url-detail-modal__panel relative flex w-full max-w-7xl max-h-[calc(100vh-2rem)] md:max-h-[calc(100vh-3rem)] flex-col overflow-hidden rounded-2xl border border-black/10 bg-white shadow-2xl dark:border-white/10 dark:bg-gray-900"
        >
          {/* Header */}
