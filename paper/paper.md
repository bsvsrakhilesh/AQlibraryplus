---
title: 'AQlibrary+: An evidence-preserving platform for governance document collection and grounded policy analysis'
tags:
  - research software
  - governance
  - policy analysis
...
Preservation is revision-based rather than link-based. Every captured artefact is associated with a canonical document and a numbered document revision. Capture records include the capture type, source URL, timestamps, cryptographic hashes, actor and request information where available, and a hashed pipeline configuration. A subsequent text extraction creates a source revision linked to the captured document revision. This deliberately separates the original bytes, derived text, and later analytical products. Local volume storage was preferred to a required cloud object store so that a research group can inspect and retain its own evidence collection. In an institutionally controlled deployment, this architecture also allows government bodies to organise restricted or non-public records without publishing them to the public. The trade-off is that access control, infrastructure security, multi-node storage, backup, and compliance policies remain deployment responsibilities rather than application-managed guarantees.

## Text extraction and tagging

Extraction follows the source format. Web pages are reduced to their principal text using document-structure and boilerplate-removal methods. PDFs are inventoried and processed page by page. Pages with sufficient native text retain that extraction, while blank or weak pages are routed automatically through OCR in bounded batches; native text on one page does not suppress OCR for a different scanned page. Each page receives a native, OCR, blank, weak, or failed state, and the document remains explicitly partial when weak or failed states remain. Extraction outputs retain page or section locators and whether OCR was used; later tags and evidence records reuse these locators so that a user can inspect the corresponding part of the stored artefact.

The baseline tagger combines normalised terms, phrase and keyphrase candidates, high-signal identifiers, and a configurable YAML taxonomy. Candidate generation is distributed across the full set of extracted page units so that later sections are not displaced by a beginning-of-document slice. The supplied air-quality taxonomy covers document types, agencies, sectors, programmes, pollutants, and canonical geography relevant to the Commission for Air Quality Management and the National Capital Region. Open-vocabulary location intelligence is extracted separately and requires evidence that explicitly names the emitted location. This deterministic path is repeatable, inspectable, and available without model credentials. When enabled, an LLM reranks candidates through bounded candidate maps and maps structured extraction over locator-preserving source windows. Each source window is processed through smaller category batches with stable operation identifiers, bounded retry, and adaptive splitting after incomplete output. Results are normalized, deduplicated, and merged without a first-N entity persistence limit rather than replacing the deterministic baseline.

Model-generated evidence must resolve to an exact source substring, for which the pipeline stores page or section context and character offsets. Categories sensitive to negation, modality, actor, status, scope, or time receive an additional automatic supported, contradicted, or insufficient classification; only supported model items are retained. Coverage is fail-closed: complete AI analysis requires every planned category-map operation and required validation batch to succeed, while deterministic-only, unavailable, and partial states remain explicit. The stored result records the tagger version, text hash, model identifier and usage status, extraction coverage, operation receipts, validation counts, rejected-item count, and supporting locators.
...
## Tagging validation scope

Automated regression tests exercise mixed native and scanned page routing, OCR workloads larger than one batch, source-order preservation, late-page entity extraction, final-window general-tag candidates, fail-closed coverage, retry receipts, dense category maps with more than 100 entities, exact source offsets, automatic contradiction rejection, uncapped persistence, and evidence-grounded open-vocabulary locations. These tests establish implementation invariants rather than empirical tagging accuracy. Quantitative precision, recall, OCR error, retrieval improvement, latency, and cost claims require a versioned representative document corpus with human-authored ground truth and are not inferred from unit-test success.
