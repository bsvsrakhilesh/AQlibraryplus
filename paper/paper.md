...
The baseline tagger combines normalised terms, phrase and keyphrase candidates, high-signal identifiers, and a configurable YAML taxonomy. Candidate generation is distributed across the full set of extracted page units so that later sections are not displaced by a beginning-of-document slice. The supplied air-quality taxonomy covers document types, agencies, sectors, programmes, pollutants, and canonical geography relevant to the Commission for Air Quality Management and the National Capital Region. Open-vocabulary location intelligence is extracted separately and requires evidence that explicitly names the emitted location. This deterministic path is repeatable, inspectable, and available without model credentials. When enabled, an LLM reranks evidence-bearing deterministic candidates and maps structured extraction over bounded windows covering the full document. Window results are evidence-gated, normalized, deduplicated, and merged rather than replacing the baseline. The stored result records the tagger version, text hash, model identifier and usage status, page coverage, map-window coverage, and supporting locators, making incomplete and model-assisted outputs distinguishable from complete rule-derived metadata.
