  ...
  try {
    ...
    if (hasPagination) {
      const out = await getUrlsPaged({
        ...
        publishedFrom:
          typeof publishedFrom === "string" ? publishedFrom : undefined,
        publishedTo: typeof publishedTo === "string" ? publishedTo : undefined,
...
export async function getUrlReviewQueueSummaryHandler(
  req: Request,
  res: Response,
  next: NextFunction,
) {
